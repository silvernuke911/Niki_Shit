...
plt.xlabel(r'Right Ascension (RA) [h]')
plt.ylabel(r'Declination (Dec) [$^\circ$]')
plt.title(r'\textbf{Star Chart [BSC]}')
dx = 10
plt.xticks(range(25)[::-1])  # Reverse labels
plt.gca().invert_xaxis()  # Reverse the x-axis
plt.yticks(np.arange(ylims[0], ylims[1]+dx, dx))

# Set background color
plt.gca().set_facecolor('black')
plt.gca().set_aspect(24/(360))

# Show the plot
plt.tight_layout()
plt.savefig(r'data\star_map_hms.png')
plt.show()